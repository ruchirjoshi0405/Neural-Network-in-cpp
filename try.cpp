...
class neural_network {
    ...
    neural_network(int num_layers, int sizes[]) {
        ...
        for(int k=0 ; k<num_layers ; k++) {
            ...
            if(k<num_layers-1) {
                init_weights_m_v(w[k], m[k], v[k], sizes[k+1], sizes[k], sizes[0]);
            }
        }
    }


    void init_weights_m_v(vector<vector<float>>& w, vector<vector<float>>& m, vector<vector<float>>& v, int x, int y, int fan_in) {
        ...
        normal_distribution<float> dist(0, 1.0);
        for(int i=0 ; i<x ; i++) {
            for(int j=0 ; j<y ; j++) {
                w[i][j] = dist(gen);
            }
...
int main() {
    ...
    dataset data("train1.csv", "test1.csv", "val.csv");
